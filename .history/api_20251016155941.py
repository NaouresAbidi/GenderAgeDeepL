...
def _load_bgr_from_bytes(content: bytes):
    ...
    if img is None:
        # Pillow fallback
        pil = Image.open(io.BytesIO(content)).convert("RGBA")  # handles PNG well
...
def _pipeline_from_bgr(bgr):
    ...
    if short_side < MIN_FACE_SIDE: quality["reasons"].append("small_face")
    if blur_var < BLUR_VAR_THRESHOLD: quality["reasons"].append("blurry")

    # --- NEW: auto-exposure on the cropped face (gentle) ---
    crop_bgr, ae_info = _auto_exposure_bgr(
        crop_bgr,
        target=0.50,       # aim for mid brightness
        tol=(0.35, 0.65),  # only correct if outside this band
        apply_clahe=False  # set True if you want a bit more pop
    )

    tensor = _bgr_to_model_tensor(crop_bgr, to_gray=(NUM_CHANNELS == 1))

    # attach what we did to quality for observability
    quality["auto_exposure"] = ae_info
    return tensor, quality

#in case of PNG files with alpha channel, we need to convert to BGR
def _to_bgr_3ch(img):
    """
...
# --- Auto-exposure helpers for brightness normalization ---

def _gamma_correct_u8(img_u8, gamma: float):
    # img_u8: uint8 [0..255], gamma > 0 darkens if >1, brightens if <1
    inv = 1.0 / 255.0
    lut = np.clip((np.arange(256) * inv) ** gamma * 255.0 + 0.5, 0, 255).astype(np.uint8)
    return cv2.LUT(img_u8, lut)

def _auto_exposure_bgr(face_bgr, target=0.50, tol=(0.35, 0.65), apply_clahe=False):
    """
    Light-touch exposure normalization on cropped face.
    - Measure mean brightness on V channel (HSV).
    - If outside tol range, apply gamma so mean -> target.
    - Optional CLAHE on L channel to recover local contrast.
    Returns adjusted BGR and a dict describing what was done.
    """
    info = {"gamma_applied": False, "gamma": 1.0, "clahe": False, "v_mean": None}
    ...
    # Only correct when clearly too bright/dark
    if v_mean < tol[0] or v_mean > tol[1]:
        eps = 1e-6
        # gamma that maps mean to target: (v_mean ** gamma) = target  => gamma = log(target)/log(v_mean)
        gamma = np.log(max(target, eps)) / np.log(max(v_mean, eps))
        # Clamp to a gentle range to avoid over-correction
        gamma = float(np.clip(gamma, 0.7, 1.4))
        face_bgr = _gamma_correct_u8(face_bgr, gamma)
        info["gamma_applied"] = True
        info["gamma"] = gamma
    ...
    if apply_clahe:
        ...
        L2 = clahe.apply(L)
        face_bgr = cv2.merge([L2, a, b])
        face_bgr = cv2.cvtColor(face_bgr, cv2.COLOR_LAB2BGR)
        info["clahe"] = True

    return face_bgr, info
